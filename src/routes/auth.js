...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (result.length === 0) {
      return res.status(400).json({ message: 'Invalid username or password', status: false });
    }
    const user = result[0];
    // Check if password is valid
    const isPasswordValid = await bcrypt.compare(password, user.password);
    if (!isPasswordValid) {
      return res.status(400).json({ message: 'Invalid userdetailsname or password', status: false });
    }
