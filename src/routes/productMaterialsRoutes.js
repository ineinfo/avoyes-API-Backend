// src/routes/productMaterialsRoutes.js
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const [existingTitle] = await pool.query(`SELECT * FROM ${TABLE.PRODUCT_MATERIALS_TABLE} WHERE status = 1 and title = ? `, [title]);
        if (existingTitle.length) {
            return res.status(400).json({ error: 'Record already exists', status: false });
        }


        await pool.query(`INSERT INTO ${TABLE.PRODUCT_MATERIALS_TABLE} (title, description) VALUES (?, ?)`, [title, description]);
...
router.get('/:id?', async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ${TABLE.PRODUCT_MATERIALS_TABLE} WHERE status = 1 and id = ?`, [id]);
            if (results.length > 0) {
        ...
        let [results] = await pool.query(`SELECT * FROM ${TABLE.PRODUCT_MATERIALS_TABLE} WHERE status = 1 ORDER BY ID DESC LIMIT ?, ?`, [offset, parsedLimit]);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [existingRecord] = await pool.query(`SELECT * FROM ${TABLE.PRODUCT_MATERIALS_TABLE} WHERE status = 1 and id = ?`, [id]);
        ...
        await pool.query(`UPDATE ${TABLE.PRODUCT_MATERIALS_TABLE} SET title = ?, description = ?, updated_at = NOW() WHERE id = ?`, [title, description, id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            await pool.query(`SELECT * FROM ${TABLE.PRODUCT_MATERIALS_TABLE} WHERE status = 1 and id = ?`, [deletedId]);
        }));

        const query = `UPDATE ${TABLE.PRODUCT_MATERIALS_TABLE} SET status = 0, deleted_at = NOW() WHERE id IN (?)`;
