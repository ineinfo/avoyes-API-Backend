...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const [existingRecord] = await pool.query(`SELECT * FROM ${TABLE.BESTFOR_TABLE} WHERE title = ?`, [title]);
        
        if (existingRecord.length > 0) {
            return res.status(400).json({ error: 'This category already exists', status: false });
        }

        await pool.query(`INSERT INTO ${TABLE.BESTFOR_TABLE} (title) VALUES (?)`, [title]);
...
router.get('/:id?', async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ${TABLE.BESTFOR_TABLE} WHERE status = 1 and id = ?`, [id]);
            if (results.length > 0) {
        ...
        let [results] = await pool.query(`SELECT * FROM ${TABLE.BESTFOR_TABLE} WHERE status = 1 ORDER BY ID DESC LIMIT ?,?`, [offset, parsedLimit]);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [existingRecord] = await pool.query(`SELECT * FROM ${TABLE.BESTFOR_TABLE} WHERE status = 1 and id = ?`, [id]);
        ...
        await pool.query(`UPDATE ${TABLE.BESTFOR_TABLE} SET title = ?, updated_at = NOW() WHERE id = ?`, [title, id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            await pool.query(`SELECT * FROM ${TABLE.BESTFOR_TABLE} WHERE status = 1 and id = ?`, [deletedId]);
        }));

        const query = `UPDATE ${TABLE.BESTFOR_TABLE} SET status = 0, deleted_at = NOW() WHERE id IN (?)`;
