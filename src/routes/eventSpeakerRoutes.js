...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const [productCheck] = await pool.query(`SELECT id FROM ${TABLE.EVENT_SPEAKER_TABLE} WHERE title = ? AND status != 0`, [title]);
        if (productCheck.length > 0) {
            return res.status(404).json({ error: 'title already exist', status: false });
        }
        
        await pool.query(
            `INSERT INTO ${TABLE.EVENT_SPEAKER_TABLE} (title,user_id) VALUES (?, ?)`,
            [title, user_id]
        );
...
router.get('/:id?', async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ${TABLE.EVENT_SPEAKER_TABLE} WHERE status !=0 and id = ?`, [id]);
            if (results.length > 0) {
        ...
        let [results] = await pool.query(`SELECT * FROM ${TABLE.EVENT_SPEAKER_TABLE} WHERE status != 0 ORDER BY id DESC LIMIT ?, ?`, [offset, parsedLimit]);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [existingRecord] = await pool.query(`SELECT * FROM ${TABLE.EVENT_SPEAKER_TABLE} WHERE status != 0 and id = ?`, [id]);
        ...
        await pool.query(`UPDATE ${TABLE.EVENT_SPEAKER_TABLE} SET title = ?, user_id = ? WHERE id = ?`, [title, user_id, id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            await pool.query(`SELECT * FROM ${TABLE.EVENT_SPEAKER_TABLE} WHERE status != 0 and id = ?`, [deletedId]);
        }));

        const query = `UPDATE ${TABLE.EVENT_SPEAKER_TABLE} SET status = 0 WHERE id IN (?)`;
